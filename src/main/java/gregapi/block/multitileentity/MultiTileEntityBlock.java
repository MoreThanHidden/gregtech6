...
public class MultiTileEntityBlock extends Block implements IBlockDebugable, IBlockErrorable, IBlockOnWalkOver, IBlockSealable, IOxygenReliantBlock, IPaintableBlock, IBlockSyncDataAndCoversAndIDs, IRenderedBlock, ITileEntityProvider, IBlockToolable, IBlockRetrievable, IBlockMaterial, IManaTrigger {
	...
	@Override public final void onBlockClicked(World aWorld, int aX, int aY, int aZ, PlayerEntity aPlayer) {TileEntity aTileEntity = aWorld.getTileEntity(aX, aY, aZ); if (aTileEntity instanceof IMTE_OnBlockClicked) ((IMTE_OnBlockClicked)aTileEntity).onBlockClicked(aPlayer); else super.onBlockClicked(aWorld, aX, aY, aZ, aPlayer);}
	...
	@Override public final ItemStack getPickBlock(MovingObjectPosition aTarget, World aWorld, int aX, int aY, int aZ, PlayerEntity aPlayer) {TileEntity aTileEntity = aWorld.getTileEntity(aX, aY, aZ); return aTileEntity instanceof IMTE_GetPickBlock?((IMTE_GetPickBlock)aTileEntity).getPickBlock(aTarget):null;}
	...
	@Override public final ArrayList<String> getDebugInfo(PlayerEntity aPlayer, int aX, int aY, int aZ, int aScanLevel) {TileEntity aTileEntity = aPlayer.worldObj.getTileEntity(aX, aY, aZ); return aTileEntity instanceof IMTE_GetDebugInfo ? ((IMTE_GetDebugInfo)aTileEntity).getDebugInfo(aScanLevel) : null;}
	...
	@Override public final boolean canHarvestBlock(PlayerEntity aPlayer, int aMeta) {return super.canHarvestBlock(aPlayer, aMeta);}
