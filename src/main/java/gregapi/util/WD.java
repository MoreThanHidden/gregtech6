...
public class WD {
	...
	public static ItemStack suck(World aWorld, double aX, double aY, double aZ) {return suck(aWorld, aPos, 1, 1, 1);}
	@SuppressWarnings("unchecked")
	public static ItemStack suck(World aWorld, double aX, double aY, double aZ, double aL, double aH, double aW) {
		for (EntityItem tItem : (ArrayList<EntityItem>)aWorld.getEntitiesWithinAABB(EntityItem.class, AxisAlignedBB.getBoundingBox(aPos, aX+aL, aY+aH, aZ+aW))) {
			if (!tItem.isDead) {
	...
	public static boolean obstructed(World aWorld, BlockPos aPos, byte aSide) {
		aX += OFFSETS_X[aSide]; aY += OFFSETS_Y[aSide]; aZ += OFFSETS_Z[aSide];
		TileEntity tTileEntity = te(aWorld, aPos, T);
		if (tTileEntity != null) {
		...
		AxisAlignedBB tBoundingBox = tBlock.getCollisionBoundingBoxFromPool(aWorld, aPos);
	...
	public static RayTraceResult getMOP(World aWorld, PlayerEntity aPlayer, boolean aFlag) {
		...
		double tReach = (aPlayer instanceof PlayerEntityMP ? ((PlayerEntityMP)aPlayer).theItemInWorldManager.getBlockReachDistance() : 5);
	...
	public static boolean move(Entity aEntity, int aDimension, double aX, double aY, double aZ) {
		...
		if (tTargetWorld != null && tOriginalWorld != null && tTargetWorld != tOriginalWorld) {
			...
			if (aEntity instanceof PlayerEntityMP) {
				PlayerEntityMP aPlayer = (PlayerEntityMP)aEntity;
			...
			if (aEntity instanceof EntityLivingBase) {
				((EntityLivingBase)aEntity).setPositionAndUpdate(aPos);
			} else {
				aEntity.setPosition(aPos);
			}
	...
	/** to get a TileEntity properly, according to my additional Interfaces. Normally you should set aLoadUnloadedChunks to false, unless you have already checked these Coordinates, or you want to load Chunks */
	public static DelegatorTileEntity<TileEntity> te(World aWorld, BlockPos aPos, byte aSide, boolean aLoadUnloadedChunks) {
		TileEntity aTileEntity = te(aWorld, aPos, aLoadUnloadedChunks);
		return aTileEntity instanceof ITileEntityDelegating ? ((ITileEntityDelegating)aTileEntity).getDelegateTileEntity(aSide) : new DelegatorTileEntity<>(aTileEntity, aWorld, aPos, aSide);
	}
	...
	/** to get a TileEntity properly, according to my additional Interfaces. Normally you should set aLoadUnloadedChunks to false, unless you have already checked these Coordinates, or you want to load Chunks */
	public static TileEntity te(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks) {
		if (aLoadUnloadedChunks || aWorld.blockExists(aPos)) {
			TileEntity rTileEntity = aWorld.getTileEntity(aPos);
			if (rTileEntity != null) return rTileEntity;
			rTileEntity = LAST_BROKEN_TILEENTITY.get();
			if (rTileEntity != null && rTileEntity.xCoord == aX && rTileEntity.yCoord == aY && rTileEntity.zCoord == aZ) return rTileEntity;
			Block tBlock = aWorld.getBlock(aPos);
			return tBlock instanceof IBlockTileEntity ? ((IBlockTileEntity)tBlock).getTileEntity(aWorld, aPos) : null;
		}
		return null;
	}
	
	/** Sets the TileEntity at the passed position, with the option of turning adjacent TileEntity updates off. */
	public static TileEntity te(World aWorld, BlockPos aPos, TileEntity aTileEntity, boolean aCauseTileEntityUpdates) {
		if (aCauseTileEntityUpdates) aWorld.setTileEntity(aPos, aTileEntity); else {
	...
	public static boolean oxygen(World aWorld, BlockPos aPos) {
		return  !MD.GC.mLoaded || !(aWorld.provider instanceof IGalacticraftWorldProvider) || OxygenUtil.checkTorchHasOxygen(aWorld, NB, aPos);
	}
	public static boolean collectable_air(World aWorld, BlockPos aPos) {
		return (!MD.GC.mLoaded || !(aWorld.provider instanceof IGalacticraftWorldProvider)) && !hasCollide(aWorld, aPos);
	}
	
	/** @return the regular Environment Temperature of the World at this Location according to my calculations. In Kelvin, ofcourse. */
	public static long envTemp(World aWorld, BlockPos aPos) {
		return envTemp(aWorld.getBiomeGenForCoords(aX, aZ), aPos);
	}
	public static long envTemp(BiomeGenBase aBiome, BlockPos aPos) {
		return Math.max(1, aBiome == null ? DEF_ENV_TEMP : (long)(C - 3 + aBiome.getFloatTemperature(aPos) * 20));
	}
	...
	public static ItemStack stack(World aWorld, BlockPos aPos) {
		Block tBlock = aWorld.getBlock(aPos);
		return ST.make(tBlock, 1, tBlock instanceof IBlockExtendedMetaData ? ((IBlockExtendedMetaData)tBlock).getExtendedMetaData(aWorld, aPos) : aWorld.getBlockMetadata(aPos));
	}
	
	public static void update(IBlockAccess aWorld, BlockPos aPos) {
		((World)aWorld).markBlockForUpdate(aPos);
		if (CLIENT_BLOCKUPDATE_SOUNDS && CODE_CLIENT && CLIENT_TIME > 100) {
			PlayerEntity tPlayer = GT_API.api_proxy.getThePlayer();
			if (tPlayer != null && Math.abs(tPlayer.posX - aX) < 16 && Math.abs(tPlayer.posY - aY) < 16 && Math.abs(tPlayer.posZ - aZ) < 16) {
				UT.Sounds.play(SFX.MC_FIREWORK_LAUNCH, 1, 1.0F, 1.0F, aPos);
			}
		}
	}
	
	public static Block block(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks) {return aLoadUnloadedChunks || aWorld.blockExists(aPos) ? aWorld.getBlock(aPos) : NB;}
	public static Block block(World aWorld, BlockPos aPos) {return aWorld.getBlock(aPos);}
	public static Block block(World aWorld, BlockPos aPos, byte aSide, boolean aLoadUnloadedChunks) {return block(aWorld, aX+OFFSETS_X[aSide], aY+OFFSETS_Y[aSide], aZ+OFFSETS_Z[aSide], aLoadUnloadedChunks);}
	public static Block block(World aWorld, BlockPos aPos, byte aSide) {return block(aWorld, aX+OFFSETS_X[aSide], aY+OFFSETS_Y[aSide], aZ+OFFSETS_Z[aSide]);}
	public static byte  meta (World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks) {return aLoadUnloadedChunks || aWorld.blockExists(aPos) ? UT.Code.bind4(aWorld.getBlockMetadata(aPos)) : 0;}
	public static byte  meta (World aWorld, BlockPos aPos) {return UT.Code.bind4(aWorld.getBlockMetadata(aPos));}
	public static byte  meta (World aWorld, BlockPos aPos, byte aSide, boolean aLoadUnloadedChunks) {return meta(aWorld, aX+OFFSETS_X[aSide], aY+OFFSETS_Y[aSide], aZ+OFFSETS_Z[aSide], aLoadUnloadedChunks);}
	public static byte  meta (World aWorld, BlockPos aPos, byte aSide) {return meta(aWorld, aX+OFFSETS_X[aSide], aY+OFFSETS_Y[aSide], aZ+OFFSETS_Z[aSide]);}
	
	public static boolean set(World aWorld, BlockPos aPos, Block aBlock, long aMeta, long aFlags) {
		return set(aWorld, aPos, aBlock, Code.bind4(aMeta), (byte)aFlags, aBlock.isOpaqueCube());
	}
	...
	public static boolean set(Chunk aChunk, BlockPos aPos, Block aBlock, long aMeta) {
	...
	public static boolean sign(World aWorld, BlockPos aPos, byte aSide, long aFlags, String aLine1, String aLine2, String aLine3, String aLine4) {
		aWorld.setBlock(aPos, Blocks.wall_sign, aSide, (byte)aFlags);
		TileEntity tSign = te(aWorld, aPos, T);
	...
	public static int random(World aWorld, BlockPos aPos, int aBound) {return random(aWorld.getSeed(), aPos, aBound);}
	public static int random(long aSeed, BlockPos aPos, int aBound) {
		Random rRandom = new Random(aSeed);
	...
	public static boolean set(World aWorld, BlockPos aPos, ItemStack aStack) {
		Block tBlock = ST.block(aStack);
		if (tBlock == NB) return F;
		if (tBlock instanceof IBlockPlacable) return ((IBlockPlacable)tBlock).placeBlock(aWorld, aPos, (byte)6, ST.meta_(aStack), aStack.getTagCompound(), T, F);
		if (ST.meta_(aStack) < 16) return aWorld.setBlock(aPos, tBlock, ST.meta_(aStack), 3);
		return F;
	}
	
	public static boolean leafdecay(World aWorld, BlockPos aPos, Block aBlock) {
		if (aBlock == null || (!(aBlock instanceof BlockBase) && aBlock.canSustainLeaves(aWorld, aPos))) {
	...
	public static boolean liquid(World aWorld, BlockPos aPos) {return liquid(aWorld.getBlock(aPos));}
	...
	public static boolean floor(World aWorld, BlockPos aPos) {return floor(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean floor(World aWorld, BlockPos aPos, Block aBlock) {return aBlock.isSideSolid(aWorld, aPos, FORGE_DIR[SIDE_UP]) || floor(aBlock);}
	...
	public static boolean visOcc(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks, boolean aDefault) {return visOpq(aWorld, aX+1, aY, aZ, aLoadUnloadedChunks || !border(aX, aZ, aX+1, aZ), aDefault) && visOpq(aWorld, aX-1, aY, aZ, aLoadUnloadedChunks || !border(aX, aZ, aX-1, aZ), aDefault) && visOpq(aWorld, aX, aY+1, aZ, T, aDefault) && visOpq(aWorld, aX, aY-1, aZ, T, aDefault) && visOpq(aWorld, aPos+1, aLoadUnloadedChunks || !border(aX, aZ, aX, aZ+1), aDefault) && visOpq(aWorld, aPos-1, aLoadUnloadedChunks || !border(aX, aZ, aX, aZ-1), aDefault);}
	public static boolean visOpq(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks, boolean aDefault) {return aLoadUnloadedChunks || aWorld.blockExists(aPos) ? visOpq(aWorld.getBlock(aPos)) : aDefault;}
	public static boolean visOpq(Block aBlock) {return aBlock.isOpaqueCube() || VISUALLY_OPAQUE_BLOCKS.contains(aBlock);}
	
	public static boolean occ(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks, boolean aDefault) {return opq(aWorld, aX+1, aY, aZ, aLoadUnloadedChunks || !border(aX, aZ, aX+1, aZ), aDefault) && opq(aWorld, aX-1, aY, aZ, aLoadUnloadedChunks || !border(aX, aZ, aX-1, aZ), aDefault) && opq(aWorld, aX, aY+1, aZ, T, aDefault) && opq(aWorld, aX, aY-1, aZ, T, aDefault) && opq(aWorld, aPos+1, aLoadUnloadedChunks || !border(aX, aZ, aX, aZ+1), aDefault) && opq(aWorld, aPos-1, aLoadUnloadedChunks || !border(aX, aZ, aX, aZ-1), aDefault);}
	public static boolean opq(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks, boolean aDefault) {return aLoadUnloadedChunks || aWorld.blockExists(aPos) ? opq(aWorld.getBlock(aPos)) : aDefault;}
	public static boolean opq(Block aBlock) {return aBlock.isOpaqueCube() && !(aBlock instanceof BlockLeaves);}
	
	public static boolean air(World aWorld, BlockPos aPos) {return air(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean air(World aWorld, BlockPos aPos, Block aBlock) {return aBlock == NB || (aBlock.isAir(aWorld, aPos) && !(MD.TC.mLoaded && !aBlock.isOpaqueCube() && te(aWorld, aPos, T) instanceof INode));}
	public static boolean air(Block aBlock) {return aBlock == NB;}
	
	public static boolean lava(IBlockAccess aWorld, BlockPos aPos) {return lava(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean lava(IBlockAccess aWorld, BlockPos aPos, Block aBlock) {return aBlock == Blocks.lava || aBlock == Blocks.flowing_lava;}
	public static boolean lava(Block aBlock) {return aBlock == Blocks.lava || aBlock == Blocks.flowing_lava;}
	
	public static boolean water(IBlockAccess aWorld, BlockPos aPos) {return water(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean water(IBlockAccess aWorld, BlockPos aPos, Block aBlock) {return aBlock == Blocks.water || aBlock == Blocks.flowing_water;}
	...
	public static boolean anywater(IBlockAccess aWorld, BlockPos aPos) {return anywater(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean anywater(IBlockAccess aWorld, BlockPos aPos, Block aBlock) {return aBlock instanceof BlockWaterlike || water(aWorld, aPos, aBlock) || waterstream(aBlock);}
	public static boolean anywater(Block aBlock) {return aBlock instanceof BlockWaterlike || water(aBlock) || waterstream(aBlock);}
	
	public static boolean bedrock(World aWorld, BlockPos aPos) {return bedrock(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean bedrock(World aWorld, BlockPos aPos, Block aBlock) {return aBlock == Blocks.bedrock || (IL.BTL_Bedrock.exists() && IL.BTL_Bedrock.equal(aBlock));}
	...
	public static boolean grass(World aWorld, BlockPos aPos, boolean aLoadUnloadedChunks) {return grass(block(aWorld, aPos, aLoadUnloadedChunks), meta(aWorld, aPos, aLoadUnloadedChunks));}
	public static boolean grass(World aWorld, BlockPos aPos) {return grass(block(aWorld, aPos), meta(aWorld, aPos));}
	public static boolean grass(World aWorld, BlockPos aPos, Block aBlock, long aMeta) {return grass(aBlock, aMeta);}
	...
	public static boolean easyRep(World aWorld, BlockPos aPos) {return easyRep(aWorld, aPos, aWorld.getBlock(aPos));}
	public static boolean easyRep(World aWorld, BlockPos aPos, Block aBlock) {return air(aWorld, aPos, aBlock) || aBlock instanceof BlockBush || aBlock.isLeaves(aWorld, aPos) || aBlock.canBeReplacedByLeaves(aWorld, aPos);}
	
	public static boolean infiniteWater(World aWorld, BlockPos aPos) {return (MD.TF.mLoaded && aWorld.provider.dimensionId == TwilightForestMod.dimensionID ? UT.Code.inside(21, 31, aY) : UT.Code.inside(52, 62, aY)) && BIOMES_RIVER_LAKE.contains(aWorld.getBiomeGenForCoords(aX, aZ).biomeName);}
	public static boolean infiniteWater(World aWorld, BlockPos aPos, Block aBlock) {return waterstream(aBlock) || ((aBlock == Blocks.water || aBlock == Blocks.flowing_water) && (MD.TF.mLoaded && aWorld.provider.dimensionId == TwilightForestMod.dimensionID ? UT.Code.inside(21, 31, aY) : UT.Code.inside(52, 62, aY)) && BIOMES_RIVER_LAKE.contains(aWorld.getBiomeGenForCoords(aX, aZ).biomeName));}
	...
	public static boolean fire(World aWorld, BlockPos aPos, boolean aCheckFlammability) {
		...
		if (tBlock.getMaterial() == Material.carpet || tBlock.getCollisionBoundingBoxFromPool(aWorld, aPos) == null) {
			if (MD.TC.mLoaded && te(aWorld, aPos, T) instanceof INode) return F;
			if (tBlock.getFlammability(aWorld, aPos, FORGE_DIR[SIDE_ANY]) > 0) return aWorld.setBlock(aPos, Blocks.fire, 0, 3);
			if (aCheckFlammability) {
				for (byte tSide : ALL_SIDES_VALID) if (block(aWorld, aPos, tSide).getFlammability(aWorld, aX+OFFSETS_X[tSide], aY+OFFSETS_Y[tSide], aZ+OFFSETS_Z[tSide], FORGE_DIR_OPPOSITES[tSide]) > 0) return aWorld.setBlock(aPos, Blocks.fire);
			} else {
				return aWorld.setBlock(aPos, Blocks.fire, 0, 3);
			}
		}
		return F;
	}
	
	public static boolean oreGenReplaceable(World aWorld, BlockPos aPos, boolean aAllowAir) {
		Block aBlock = aWorld.getBlock(aPos);
		if (aBlock == NB) return aAllowAir;
		byte aMetaData = (byte)aWorld.getBlockMetadata(aPos);
		...
		if (Blocks.stone        != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.stone        )) return T;
		if (Blocks.gravel       != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.gravel       )) return T;
		if (Blocks.sand         != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.sand         )) return T;
		if (Blocks.netherrack   != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.netherrack   )) return T;
		if (Blocks.end_stone    != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.end_stone    )) return T;
		return F;
	}
	
	public static boolean setOre(World aWorld, BlockPos aPos, OreDictMaterial aMaterial) {
		return aMaterial != null && setOre(aWorld, aPos, aMaterial.mID);
	}
	
	public static boolean setOre(World aWorld, BlockPos aPos, short aID) {
		...
		if (tBlock == null) {
		if (Blocks.stone        != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.stone        )) tBlock = BlocksGT.ore; else
		if (Blocks.gravel       != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.gravel       )) tBlock = BlocksGT.oreGravel; else
		if (Blocks.sand         != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.sand         )) tBlock = BlocksGT.oreSand; else
		if (Blocks.netherrack   != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.netherrack   )) tBlock = BlocksGT.oreNetherrack; else
		if (Blocks.end_stone    != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.end_stone    )) tBlock = BlocksGT.oreEndstone;
		}
		return tBlock != null && tBlock.placeBlock(aWorld, aPos, (byte)6, aID, null, F, T);
	}
	
	public static boolean setSmallOre(World aWorld, BlockPos aPos, OreDictMaterial aMaterial) {
		return aMaterial != null && setSmallOre(aWorld, aPos, aMaterial.mID);
	}
	
	public static boolean setSmallOre(World aWorld, BlockPos aPos, short aID) {
		if (aID <= 0 && aID == W) return F;
		Block aBlock = aWorld.getBlock(aPos);
		if (aBlock == NB) return F;
		byte aMetaData = (byte)aWorld.getBlockMetadata(aPos);
		...
		if (tBlock == null) {
		if (Blocks.stone        != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.stone        )) tBlock = BlocksGT.oreSmall; else
		if (Blocks.gravel       != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.gravel       )) tBlock = BlocksGT.oreSmallGravel; else
		if (Blocks.sand         != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.sand         )) tBlock = BlocksGT.oreSmallSand; else
		if (Blocks.netherrack   != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.netherrack   )) tBlock = BlocksGT.oreSmallNetherrack; else
		if (Blocks.end_stone    != aBlock && aBlock.isReplaceableOreGen(aWorld, aPos, Blocks.end_stone    )) tBlock = BlocksGT.oreSmallEndstone;
		}
		return tBlock != null && tBlock.placeBlock(aWorld, aPos, (byte)6, aID, null, F, T);
	}
	
	/** Removes Bedrock from that Position and replaces it with regular Stone of the region. */
	public static boolean removeBedrock(World aWorld, BlockPos aPos) {
		Block tBlock = aWorld.getBlock(aPos), tStone = (aWorld.provider.dimensionId == DIM_NETHER ? Blocks.netherrack : Blocks.stone);
			...
			for (int i = 1; i < 7; i++) for (byte tSide : ALL_SIDES_BUT_BOTTOM) {
				...
				if (tBlock != NB && tBlock != tStone && !bedrock(tBlock)) {
					...
					if (BlocksGT.stoneToNormalOres.containsKey(new ItemStackContainer(tBlock, 1, tMetaData))) {
						return aWorld.setBlock(aPos, tBlock, tMetaData, 0);
					}
				}
			}
			return aWorld.setBlock(aPos, tStone, 0, 0);
		}
		return F;
	}
	
	public static long scan(ArrayList<String> aList, PlayerEntity aPlayer, World aWorld, int aScanLevel, BlockPos aPos, byte aSide, float aClickX, float aClickY, float aClickZ) {
		...
		Block aBlock = aWorld.getBlock(aPos);
		byte aMeta = (byte)aWorld.getBlockMetadata(aPos);
		TileEntity aTileEntity = te(aWorld, aPos, T);
		...
		try {
			...
			float tResistance = aBlock.getExplosionResistance(aPlayer, aWorld, aPos, aPlayer.posX, aPlayer.posY, aPlayer.posZ);
			rList.add("Hardness: " + aBlock.getBlockHardness(aWorld, aPos) + "  Blast Resistance: " + tResistance + (tResistance < 16 ? " (TNT Blastable)" : " (TNT Proof)"));
			...
			if (aBlock.isBeaconBase(aWorld, aPos, aX, aY+1, aZ)) rList.add("Is usable for Beacon Pyramids");
			if (MD.GC.mLoaded && aBlock instanceof IPartialSealableBlock) rList.add(((IPartialSealableBlock)aBlock).isSealed(aWorld, aPos, FORGE_DIR[aSide ^ 1]) ? "Is Sealable on this Side" : "Is not Sealable on this Side");
		...
		try {if (aBlock instanceof IBlockDebugable) {
			rEUAmount+=V[3];
			ArrayList<String> temp = ((IBlockDebugable)aBlock).getDebugInfo(aPlayer, aPos, aScanLevel);
			if (temp != null) rList.addAll(temp);
		}} catch(Throwable e) {if (D1) e.printStackTrace(ERR);}
		
		BlockScanningEvent tEvent = new BlockScanningEvent(aWorld, aPlayer, aPos, aSide, aScanLevel, aBlock, aTileEntity, rList, aClickX, aClickY, aClickZ);
