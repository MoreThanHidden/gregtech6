		...
		addListener(OD.logWood, OD.logRubber, new IOreDictListenerEvent() {@Override public void onOreRegistration(OreDictRegistrationContainer aEvent) {
			...
			if (ST.meta_(aEvent.mStack) == W) {
				...
				for (int i = 0; i < W; i++) {
					...
					tPlanks.getCount() = (tPlanks.getCount() * 3) / 2;
					...
					CR.shaped(ST.amount(NERFED_WOOD?tPlank.getCount():(tPlank.getCount() * 5) / 4, tPlank), CR.DEF_NAC_NCC | CR.ONLY_IF_HAS_RESULT, "s", "L", 'L', ST.make(ST.item_(aEvent.mStack), 1, i));
					CR.shapeless(ST.amount(tPlank.getCount() / (NERFED_WOOD?2:1), tPlank), CR.DEF_NAC_NCC | CR.ONLY_IF_HAS_RESULT, new Object[] {ST.make(ST.item_(aEvent.mStack), 1, i)});
				}
			} else {
				...
				if (tPlank != null) {
					...
					CR.shaped(ST.amount(NERFED_WOOD?tPlank.getCount():(tPlank.getCount() * 5) / 4, tPlank), CR.DEF_NAC_NCC | CR.ONLY_IF_HAS_RESULT, "s", "L", 'L', aEvent.mStack);
					CR.shapeless(ST.amount(tPlank.getCount() / (NERFED_WOOD?2:1), tPlank), CR.DEF_NAC_NCC | CR.ONLY_IF_HAS_RESULT, new Object[] {aEvent.mStack});
				}
			}
		}});
